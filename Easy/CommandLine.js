...
function CommandLine(str) {
  ...
  for (const char of strSlices) {
    if (char.includes('=')) {
      resultArrIdx += 1;
      resultArr[resultArrIdx] = char;
    } else {
      resultArr[resultArrIdx] = `${resultArr[resultArrIdx]} ${char}`;
    }
  }
  return resultArr
    .map((item) => {
...
console.log(CommandLine('letters=A B Z T numbers=1 2 26 20 combine=true')); // Output: 7=7 7=9 7=4
console.log(CommandLine('a=3 b=4 a=23 b=a 4 23 c=')); // Output: 1=1 1=1 1=2 1=6 1=0
